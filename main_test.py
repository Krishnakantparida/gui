...
def _render_legend(model) -> None:
    ...
    with legend_container:
        ...
        if model.engines:
            ...
            # HD Engines checkbox (color-coded differently for pure silicon)
...
def _update_toggle_buttons() -> None:
    ...
    if not history or idx < 0:
        ...
        return
    test_toggle_left.props(remove="disabled") if idx > 0 else test_toggle_left.props("disabled")
    test_toggle_right.props(remove="disabled") if idx < len(history) - 1 else test_toggle_right.props("disabled")
...
def _on_toggle_left() -> None:
    """Legacy: no-op (arrows moved to test display)."""
    pass
...
# Legacy toggle handlers (arrows moved to test display beside Run button).
# The test_toggle_left/right buttons are created in the UI below and
# have their click handlers assigned there.
